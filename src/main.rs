...
/// This is a linux multithreadded application.
/// The relationship state machine statuses are expresed as log events in the CLI.
...
impl Relationship {
    ...
    fn apply_delta(&self, delta:i8, actor:&str, description: &str) {
        ...
        *health = (*health + delta).clamp(0, 100);
        // force the result into the [0, 100] range. If health+delta is -5, this returns 0. If it is 130, returns 100. Prevents health going out of bounds.
...
impl Actor for Boyfriend {
    fn act(&self, rel: &Relationship) {
        let (delta, description) =  match rand::random_range(0..3) {  // max range = Boyfriend number of impl fn, as range is exclusive
            ...
            _ => unreachable!("Actor for Boyfriend: Incorrect range of state list. Must be 0..3."),  // range is 0..3, compiler can not prove exhaustiveness itself
        };
...
impl Actor for Girlfriend {
    fn act(&self, rel: &Relationship) {
        let (delta, description) =  match rand::random_range(0..3) {  // max range = Girlfriend number of impl fn, as range is exclusive
            ...
            _ => unreachable!("Actor for Girlfriend: Incorrect range of state list. Must be 0..3."),  // range is 0..3, compiler can not prove exhaustiveness itself
        };
...
impl Actor for OtherHotGuy {
    fn act(&self, rel: &Relationship) {
        let (delta, description) =  match rand::random_range(0..2) {  // max range = OtherHotGuy number of impl fn, as range is exclusive
            ...
            _ => unreachable!("Actor for OthoerHotGuy: Incorrect range of state list. Must be 0..2."),  // range is 0..2, compiler can not prove exhaustiveness itself
        };
